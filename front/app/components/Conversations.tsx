...
import {
    MessageSquare, Users, Search, Mail, Phone, Video, Circle,
    Clock, ArrowRight, MessageCircle, Plus, Shield
} from 'lucide-react';
...
export const Conversations = ({
	...
	// Amélioration du filtrage des utilisateurs
	const usersExceptMe = users.filter(u => u.id !== connectedUser.id);
	const usersWithoutConversation = usersExceptMe.filter(u => 
		!conversations.some(c => c.users.some(cu => cu.id === u.id))
	);

	// Tri des conversations par date
				...
				<section className="bg-white/5 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
					<div className="flex items-center justify-between mb-6">
						...
						<span className="text-xs text-gray-500 bg-white/5 px-3 py-1 rounded-full">
							{usersWithoutConversation.length} utilisateurs disponibles
						</span>
					</div>
...
// ConversationItem avec props typées
